...
{
	backgroundImage = LoadBitmap(hInst, MAKEINTRESOURCE(IDB_LOBBY));						// ¹è°æ ÀÌ¹ÌÁö

	selecMapImage = LoadBitmap(hInst, MAKEINTRESOURCE(IDB_SELECTMAP));						// ¸Ê ¼±ÅÃ ÀÌ¹ÌÁö
...
void CLobbyScene::ProcessInput()
{
	...
	if (pKeysBuffer[VK_RETURN] & 0xF0) GetFramework()->SetCurScene(PLAYSCENE);			// PlaySceneÀ¸·Î ³Ñ¾î°£´Ù.
}
...
void CLobbyScene::Render()
{
	...
	OldBit[1] = (HBITMAP)SelectObject(MemDCImage, backgroundImage); // ¹è°æ ÀÌ¹ÌÁö	
	...
	if (recvLobbyPacket.playerExist[0]) {
		OldBit[1] = (HBITMAP)SelectObject(MemDCImage, playerImage[0]); // ¹è°æ ÀÌ¹ÌÁö	
		StretchBlt(MemDC, 43, 150, rc.right, rc.bottom, MemDCImage, 0, 0, 1220, 950, SRCCOPY);
	}
	if (recvLobbyPacket.playerExist[1]) {
		OldBit[1] = (HBITMAP)SelectObject(MemDCImage, playerImage[1]); // ¹è°æ ÀÌ¹ÌÁö	
		StretchBlt(MemDC, 0, 0, rc.right, rc.bottom, MemDCImage, 0, 0, 1220, 950, SRCCOPY);
	}
...
void CLobbyScene::SendData()
{
	...
	if (pause == false || pastData != sendLobbyPacket.selectedMap || sendLobbyPacket.nextSceneCall != pastCallData) {
